 ...
 *   - simplecrawlerOpt {object} : config options for node-simplecrawler 
 *   - ignoreHttpError {Boolean}: flag to ignore http error response codes (ie. 404), default = false 
 *   - fetchConditions {Array}: array of node-simplecrawler fetch condition functions to apply
 *
...
crawler.crawl = function(url, options, callback) {
  ...
  _.defaults(options.simplecrawlerOpt, {
    interval: 10,
    maxConcurrency: 10,
    scanSubdomains: true,
  ...
  // do a web crawl of url if it's an http protocol
	if (urlParts.protocol == "https:" || urlParts.protocol == "http:") {
        ...
        for (var k in options.simplecrawlerOpt) siteCrawler[k] = options.simplecrawlerOpt[k];
        for (var i = 0; i < options.fetchConditions.length; i++){
            siteCrawler.addFetchCondition(options.fetchConditions[i]);
        }
